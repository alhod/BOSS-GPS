...
class dijkstrasTour:
    ...
    def run(self, startingRoom, endingRoom, walkingSpeed, roomsToVisit):
        ...
        self.finalEndingRoom = endingRoom
        if self.finalEndingRoom in self.specialRoomToSpecialRoomNumber:
            self.finalEndingRoom = self.specialRoomToSpecialRoomNumber[self.finalEndingRoom]
        elif self.finalEndingRoom in self.roomsWithMultipleEntrances:
            self.finalEndingRoom = self.finalEndingRoom+"A"
        self.finalEndingRoom = self.roomToIndex[self.finalEndingRoom]

        self.roomsToVisit = roomsToVisit

        for i in range(0, len(self.roomsToVisit)):
            if self.roomsToVisit[i] in self.specialRoomToSpecialRoomNumber:
                self.roomsToVisit[i] = self.specialRoomToSpecialRoomNumber[self.roomsToVisit[i]]
            elif self.roomsToVisit[i] in self.roomsWithMultipleEntrances:
                self.roomsToVisit[i] = self.roomsToVisit[i]+"A"
            self.roomsToVisit[i] = self.roomToIndex[self.roomsToVisit[i]]

        self.walkingSpeed = (float(walkingSpeed)*1000)/3600
        tourPaths = []
        
        while len(self.roomsToVisit) > 0:
            dist, path = self.getShortestPath(self.roomToConnectedRooms, self.numRooms, self.startingRoom, self.roomsToVisit)
            tourPaths.append(self.translatePath(dist, path))
            self.startingRoom = self.endingRoom
            self.endingRoom = None

        self.roomsToVisit.append(self.finalEndingRoom)
        dist, path = self.getShortestPath(self.roomToConnectedRooms, self.numRooms, self.startingRoom, self.roomsToVisit)
        tourPaths.append(self.translatePath(dist, path))
    ...
    def SSSP(self, roomToConnectedRooms, numRooms, startingRoom, roomsToVisit):
        visited = {}
        ...
        while len(pq) > 0:
            ...
            for edge in roomToConnectedRooms[index]:
                if self.indexToRoom[edge[1]][0] in '12' and edge[1] not in self.roomsToVisit and self.indexToRoom[edge[1]][:4] != self.indexToRoom[self.startingRoom][:4]:
                    continue
            ...
            if index in self.roomsToVisit:
                self.roomsToVisit.remove(index)
    ...
    def getShortestPath(self, roomToConnectedRooms, numRooms, startingRoom, roomsToVisit):
        dist, previous = self.SSSP(roomToConnectedRooms, numRooms, startingRoom, roomsToVisit)
        path = []
    ...
    def translatePath(self, dist, path):
        ...
        while True:
            ...
            ind+=1
        if self.endingRoom == self.finalEndingRoom:
            finalMessage.append([f"You have reached room {self.indexToRoom[self.endingRoom]}. The tour is finished!", self.roomToPictures[self.endingRoom]])
        else:
